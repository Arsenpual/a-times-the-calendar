  ...
  return <div className={`activity-ai-backdrop${activityFormOpen ? " is-activity-form-open" : ""}${dailySummaryOpen ? " is-daily-summary-open" : ""}`} role="presentation" onMouseDown={onClose}>
    <section className="activity-ai-assistant" role="dialog" aria-modal="true" aria-label="คุยกับ MR.Zettascale เพื่อสร้างกิจกรรม" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <main className="activity-ai-messages">
        ...
        <section className="activity-ai-centered-questions activity-ai-calendar-facts" aria-label="ข้อมูลจาก Google Calendar"><small>ข้อมูลจาก Google Calendar · ไม่ใช้ AI quota · อ่านอย่างเดียว</small><div>{CALENDAR_FACT_SUGGESTIONS.map((question) => <button key={question} type="button" onClick={() => send(null, question)} disabled={pending}>{question}</button>)}</div></section>
        <section className="activity-ai-centered-questions activity-ai-calendar-questions" aria-label="ให้ AI ช่วยวางแผนจาก Google Calendar"><small>ให้ AI ช่วยวิเคราะห์ตาราง · ใช้ AI quota 1 ครั้ง · อ่านอย่างเดียว</small><div>{CALENDAR_AI_QUESTION_SUGGESTIONS.map((question) => <button key={question} type="button" onClick={() => send(null, question)} disabled={pending || aiStatus?.enabled === false || cooldownSeconds > 0}>{question}</button>)}</div></section>
